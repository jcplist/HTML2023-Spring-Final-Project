...
def neural (y, x):
    ...
    m = Sequential()
    m.add(LSTM(units = 1000, return_sequences = True, input_shape = (X_train.shape[1], 1)))
    m.add(Dropout(0.2))
    m.add(LSTM(units = 500, return_sequences = True))
    m.add(Dropout(0.2))
    m.add(LSTM(units = 200, return_sequences = True))
    m.add(Dropout(0.2))
    m.add(LSTM(units = 20))
    m.add(Dropout(0.2))
    m.add(Dense(units = 1))
...
'''
Epoch 53/5000
310/310 [==============================] - 5s 17ms/step - loss: 1.5681 - val_loss: 1.7821
'''
