...
def bagging ():
    ...
    for _ in range(12000):
        ty, tx = random_select(1000, rng)
        regr = svm_train(ty, tx, "-s 3 -t 2 -c 10 -g 0.5 -e 0.00001 -h 0 -m 8192 -q")
        vy, q, qq = svm_predict(train_y, train_x, regr, "-q")
        vy = mapping(vy)
        print(f"Err {_:5d}:", sum([abs(vy[i] - train_y[i]) for i in range(len(train_y))]) / len(train_y))
        models.append(regr)
    return models
...
p_labels = np.zeros(len(test_x))

for regr in m:
    py, q, qq = svm_predict([], test_x, regr, "-q")
    p_labels += np.array(py)
