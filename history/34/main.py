...
def scaling (x):
    ...
    for i in range(len(x)):
        ...
        x[i][6] = min(x[i][6], 195.001) #Q99
        x[i][6] /= 243

        x[i][7] /= 10

        if len(x[i]) >= 11:
            x[i][9] = min(x[i][9], -2.161) #Q99
            x[i][10] = min(x[i][10], 474302) #Q99
...
rng = np.random.default_rng(seed=0xCC12)
rng.shuffle(train_x)
rng = np.random.default_rng(seed=0xCC12)
rng.shuffle(train_y)
...
for i in range(len(p_labels1)):
    p_labels[i] = (p_labels1[i] + p_labels2[i]) / 2
